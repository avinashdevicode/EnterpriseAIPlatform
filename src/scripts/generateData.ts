...
// Country configuration with currencies and languages
const countries = [
...
// Generate sample PDF URLs
const generatePDFUrls = (count: number): string[] => {
  return Array.from({ length: count }, (_, i) => 
    `https://www.w3.org/WAI/ER/tests/xhtml/testfiles/resources/pdf/dummy.pdf?id=${i + 1}`
  );
};
...
const generateCustomers = (count: number) => {
  const customers = [];
  const pdfUrls = generatePDFUrls(50);
...
// Generate invoices
const generateInvoices = (count: number, customers: any[]) => {
  const invoices = [];
  const pdfUrls = generatePDFUrls(50);
  ...
  for (let i = 0; i < count; i++) {
    ...
    // Generate activity timeline based on stage
    ...
    invoices.push({
      ...
      pdfUrl: faker.helpers.arrayElement(pdfUrls),
      extractedFields: {
...
const generateAgentMetrics = (count: number) => {
  const metrics = [];
  ...
  for (let i = 0; i < count; i++) {
    ...
    metrics.push({
      ...
      averageProcessingTime: faker.number.int({ min: 30, max: 300 }), // seconds
    });
  }
...
const generateAllData = () => {
  ...
  const invoices = generateInvoices(1000, customers);
  console.log(`✓ Generated ${invoices.length} invoices`);
  
  const agentMetrics = generateAgentMetrics(1000);
  console.log(`✓ Generated ${agentMetrics.length} agent metrics`);
  ...
  // Write to file
  const filePath = path.join(dataDir, 'mockData.json');
  ...
  console.log(`  - Agent Metrics: ${agentMetrics.length}`);
